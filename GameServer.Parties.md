# `GameServer.Parties`
[🔗](https://github.com/appsinacup/game_server/blob/v1.0.7/lib/game_server/parties.ex#L1)
...
```elixir
@spec cancel_party_invite(GameServer.Accounts.User.t(), Ecto.UUID.t()) ::
  :ok | {:error, atom()}
```
...
```elixir
@spec decline_party_invite(GameServer.Accounts.User.t(), Ecto.UUID.t()) ::
  :ok | {:error, atom()}
```
...
```elixir
@spec get_party_members(GameServer.Parties.Party.t() | Ecto.UUID.t()) :: [
