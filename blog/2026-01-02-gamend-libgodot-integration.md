...
The **Simple Function** is an HTTP call with Authentication, while the **Complex Function** one does also database access or cache access.
...
How does it work? It starts godot and calls **iteration** as to move the engine **process** function forward.
...
Since Elixir runs its application inside BEAM and that doesn't offer access to the **main thread** (and Godot really wants to run on the main thread), I ended up starting a thread and running there Godot. That means Godot won't be able to load menus (and possibly other stuff) that require main thread. This is fine for my use cases, as I run it **headless** anyway.
...
And the time is much better than before, in the order of 0.1 - 0.5 ms, closer to what Elixir was obtaining:
