defmodule Gamend.Cache do
  ...
  runs the function, stores the result, and returns it:
      ...
      defmodule Gamend.Modules.MyPlugin do
        use Gamend.Hooks

        def generate_cheatsheet(lang) do
          Gamend.Cache.cached({:my_plugin, :cheatsheet, lang}, ttl: :timer.minutes(30), fn ->
  ...
  Equivalent to:
      ...
      case Gamend.Cache.get(key) do
        nil ->
          result = fun.()
          Gamend.Cache.put(key, result, opts)
          result
      ...
      def my_hook(lang) do
        ...
        case Gamend.Cache.get(cache_key) do
          nil ->
            ...
            Gamend.Cache.put(cache_key, result, ttl: :timer.minutes(10))
            {:ok, result}
  ...
  so the next read produces a cache miss:
      ...
      defp my_version(entity_id) do
        Gamend.Cache.get({:my_plugin, :version, entity_id}) || 1
      end

      defp invalidate(entity_id) do
        Gamend.Cache.incr({:my_plugin, :version, entity_id}, 1, default: 1)
      end
      ...
      def get_data(entity_id) do
        ...
        case Gamend.Cache.get(cache_key) do
          nil ->
            ...
            Gamend.Cache.put(cache_key, data, ttl: :timer.minutes(5))
            {:ok, data}
  ...
  **Note:** This is an SDK stub. The actual implementation uses Nebulex on
  ...
  `get` + `put` in a single call:
      ...
      Gamend.Cache.cached({:my_plugin, :data, id}, ttl: :timer.minutes(10), fn ->
  ...
  defp stub!(fun) do
    case Application.get_env(:gamend_sdk, :stub_mode, :raise) do
      :placeholder -> nil
      _ -> raise "Gamend.Cache.#{fun} is a stub - only available at runtime on Gamend"
    end
  end
end
