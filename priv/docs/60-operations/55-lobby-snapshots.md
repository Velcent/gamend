...
- **After every hook call.** The snapshot is taken against the hook caller's current lobby and attributed `hook:<name>` — and since externally-driven hooks like `after_user_offline` go through the same path, a player dropping offline mid-run leaves a snapshot too.
