...
If you want to use `CACHE_L2=partitioned` under Compose, you also need to configure Erlang distribution + node discovery for your app containers.
...
```bash
RATE_LIMIT_HTTP_GENERAL_LIMIT=240
...
When a client exceeds the limit, the server returns HTTP 429 Too Many Requests with a Retry-After header. With the default ETS backend each node counts separately; set RATE_LIMIT_BACKEND=redis so limits hold across all instances.
