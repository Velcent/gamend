...
On the local backend, objects are served by `GET /storage/*key` from the app itself; on S3 the object URL points at the bucket (or at `GAMEND_STORAGE_PUBLIC_URL` when set) and that route is unused. Cache policy is keyed by prefix: `avatars/` and `icons/` are immutable for a year — their URL is content-unique because every change mints a new key — and everything else revalidates via ETag. The serve route labels only real image bytes as images; any other stored type comes back as an opaque download, never rendered from the app's origin.
