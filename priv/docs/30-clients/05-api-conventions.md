...
Tournaments accept either a UUID or a slug in the `:id` path segment and resolve the slug to the current occurrence — `/api/v1/tournaments/weekend_gauntlet` stays a valid link from one week to the next. Leaderboards filter by slug instead: `GET /api/v1/leaderboards?slug=weekly_kills&active=true`.
...
Reads return the payload under `data`. Paginated lists add `meta`, always with the same six keys — no endpoint omits any of them:
