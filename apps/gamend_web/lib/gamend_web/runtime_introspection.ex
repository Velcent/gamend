defmodule GamendWeb.RuntimeIntrospection do
  @moduledoc """
  ...
  alias Gamend.Notifications.Types, as: NotificationTypes
  ...
  @sdk_hooks_path Path.expand("../../../../sdk/lib/gamend/hooks.ex", __DIR__)
  ...
  def hooks do
    ...
    Gamend.Hooks.behaviour_info(:callbacks)
    |> Enum.sort()
    |> Enum.map(fn {name, arity} ->
      ...
      kind = if Gamend.Hooks.pipeline_hook?(name, arity), do: "pipeline", else: "fanout"
  ...
  def protobuf_messages do
    server = Enum.map(app_modules(:gamend_web), &{&1, "server"})
  ...
  def channels do
    for {pattern, module, description} <- GamendWeb.UserSocket.__channels__() do
      %{
        id: pattern,
        pattern: pattern,
  ...
  def events do
    core =
      for entry <- GamendWeb.RealtimeEvents.all() do
  ...
  def data_model do
    server =
      for mod <- ecto_schemas(app_modules(:gamend_core)),
          do: {mod, schema_domain(mod), "server"}

    # The host app (:gamend_host) is the running game; its own Ecto schemas
    # (a game's custom tables) live here, not in core or a plugin, so they were
    # previously missing from the model entirely.
    host =
      for mod <- ecto_schemas(app_modules(:gamend_host)),
  ...
  # (Gamend.Matchmaking.Ticket -> "Matchmaking"); two-part modules
  # (Gamend.OAuthSession) are their own domain. Plugin schemas do not go
  ...
  @doc "Plugin-registered scheduled jobs (see `Gamend.Schedule`)."
