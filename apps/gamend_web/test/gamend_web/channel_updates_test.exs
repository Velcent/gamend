defmodule GamendWeb.ChannelUpdatesTest do
  @moduledoc """
  ...
  setup tags do
    ...
    prev = Application.get_env(:gamend_web, GamendWeb.Realtime)

    on_exit(fn ->
      if prev,
        do: Application.put_env(:gamend_web, GamendWeb.Realtime, prev),
        else: Application.delete_env(:gamend_web, GamendWeb.Realtime)
    end)

    {:ok, _, socket} =
      socket(GamendWeb.UserSocket, "cu", %{})
  ...
  defp set_debounce(ms),
    do: Application.put_env(:gamend_web, GamendWeb.Realtime, debounce_ms: ms)
