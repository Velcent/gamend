defmodule GamendWeb.Plugs.LoadThemeTest do
  use GamendWeb.ConnCase, async: false

  alias Gamend.Content
  alias Gamend.Theme.JSONConfig
  alias GamendWeb.Plugs.LoadTheme
  ...
  test "uses generic missing-theme fallback when provider returns empty map", %{conn: conn} do
    orig_mod = Application.get_env(:gamend_web, :theme_module)
    Application.put_env(:gamend_web, :theme_module, __MODULE__.EmptyThemeMock)

    on_exit(fn ->
      if orig_mod,
        do: Application.put_env(:gamend_web, :theme_module, orig_mod),
        else: Application.delete_env(:gamend_web, :theme_module)
    end)
  ...
  test "hands the request locale to the theme provider", %{conn: conn} do
    orig_mod = Application.get_env(:gamend_web, :theme_module)
    Application.put_env(:gamend_web, :theme_module, __MODULE__.LocaleEchoMock)
