defmodule GamendWeb.PaymentDownloadControllerTest do
  ...
  setup do
    original_dir = Application.get_env(:gamend_web, :payment_downloads_dir)

    downloads_dir =
      Path.join(System.tmp_dir!(), "payment-downloads-#{System.unique_integer([:positive])}")

    File.mkdir_p!(downloads_dir)
    Application.put_env(:gamend_web, :payment_downloads_dir, downloads_dir)
  ...
  defp restore_env(key, nil), do: Application.delete_env(:gamend_web, key)
