defmodule GamendWeb.AdminLive.ConfigTest do
  ...
  test "secrets are masked and DB adapter shows Postgres when env is set", %{conn: conn} do
    ...
    Gamend.SettingsHelpers.put(:gamend_core, Gamend.Mail, :smtp_username, "smtpuser")
    Gamend.SettingsHelpers.put(:gamend_core, Gamend.Mail, :smtp_password, "smtppass")
    Gamend.SettingsHelpers.put(:gamend_core, Gamend.Mail, :smtp_port, 465)
    Gamend.SettingsHelpers.put(:gamend_core, Gamend.Mail, :smtp_ssl, true)
    Gamend.SettingsHelpers.put(:gamend_core, Gamend.Mail, :smtp_tls, true)
    ...
    previous_settings =
      Map.new(
        [
          ...
          Gamend.Database,
          Gamend.Accounts,
          GamendWeb.Observability
        ],
        &{&1, Application.get_env(:gamend_core, &1)}
      )
    ...
    on_exit(fn ->
      ...
      Enum.each(previous_settings, fn {module, value} ->
        if value,
          do: Application.put_env(:gamend_core, module, value),
          else: Application.delete_env(:gamend_core, module)
      end)
    end)
  ...
  test "clicking function name pre-fills function and example args", %{conn: conn} do
    ...
    hook_mod = Module.concat([Gamend, AdminConfigTestPluginHook])
    ...
    orig_plugins_dir =
      Gamend.SettingsHelpers.get(:gamend_core, Gamend.ContentSettings, :plugins_dir)
