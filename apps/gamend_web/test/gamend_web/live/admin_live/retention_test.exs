defmodule GamendWeb.AdminLive.RetentionTest do
  ...
  test "renders the retention status and configured windows", %{conn: conn, admin: admin} do
    {:ok, _view, html} = conn |> log_in_user(admin) |> live(~p"/admin/retention")

    assert html =~ "Data Retention"
    assert html =~ "RETENTION_*"
    assert html =~ "never"
    # every declared window is listed with its env var name
    assert html =~ "GAMEND_RETENTION_CHAT_MESSAGES_DAYS"
    assert html =~ "GAMEND_RETENTION_PUSH_TOKENS_DAYS"
  end
  ...
  test "Run now reports a sweeper that is not running", %{conn: conn, admin: admin} do
    {:ok, view, _html} = conn |> log_in_user(admin) |> live(~p"/admin/retention")
