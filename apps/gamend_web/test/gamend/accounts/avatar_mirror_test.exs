defmodule Gamend.Accounts.AvatarMirrorTest do
  ...
  setup do
    ...
    Application.put_env(:gamend_core, :avatar_mirror_req_options, plug: {Req.Test, __MODULE__})
    ...
    on_exit(fn ->
      ...
      Application.delete_env(:gamend_core, :avatar_mirror_req_options)
    end)
  ...
  describe "a mirror that fails is not permanent" do
    ...
    test "a discarded attempt does not block the next one from being enqueued" do
      ...
      {:ok, _} = first |> Ecto.Changeset.change(state: "discarded") |> Gamend.Repo.update()
