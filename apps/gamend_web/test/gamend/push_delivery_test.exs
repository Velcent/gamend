defmodule Gamend.PushDeliveryTest do
  use Gamend.DataCase, async: false
  use Oban.Testing, repo: Gamend.Repo

  alias Gamend.AccountsFixtures
  alias Gamend.Push
  alias Gamend.Push.APNSDispatcher
  alias Gamend.Push.DeliveryWorker
  alias Gamend.Push.FanoutWorker
  alias Gamend.Push.Message
  alias Gamend.Push.Providers.APNs
  alias Gamend.Push.Providers.FCM
  alias Gamend.Push.Providers.Log
  ...
  defmodule InvalidProvider do
    @behaviour Gamend.Push.Provider
    @impl true
  ...
  defp put_push_env(overrides) do
    old = Application.get_env(:gamend_core, Gamend.Push, [])
    Application.put_env(:gamend_core, Gamend.Push, Keyword.merge(old, overrides))
    on_exit(fn -> Application.put_env(:gamend_core, Gamend.Push, old) end)
  end
  ...
  describe "Message.new/1" do
    ...
    test "rejects missing or oversized fields" do
      ...
      long_title = String.duplicate("x", Gamend.Limits.get(:max_push_title) + 1)
      assert {:error, %{title: _}} = Message.new(%{title: long_title})

      big_value = String.duplicate("x", Gamend.Limits.get(:max_push_data_size) + 1)
    ...
    test "caps are bytes, not characters" do
      ...
      max_body = Gamend.Limits.get(:max_push_body)
