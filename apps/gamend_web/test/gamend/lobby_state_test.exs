defmodule Gamend.LobbyStateTest do
  use Gamend.DataCase
  ...
  defmodule StateHook do
    def before_lobby_state_change(_lobby, from, to) do
      send(Application.get_env(:gamend, :hooks_test_pid), {:before_state, from, to})
      if to == "playing", do: {:error, :not_ready}, else: :ok
    end

    def after_lobby_state_changed(_lobby, from, to) do
      send(Application.get_env(:gamend, :hooks_test_pid), {:after_state, from, to})
      :ok
    end
  end
  ...
  defp with_state_hook(fun) do
    ...
    Application.put_env(:gamend_core, :hooks_module, StateHook)
    ...
    after
      if orig,
        do: Application.put_env(:gamend_core, :hooks_module, orig),
