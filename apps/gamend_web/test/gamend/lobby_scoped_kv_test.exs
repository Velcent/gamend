defmodule Gamend.LobbyScopedKvTest do
  @moduledoc """
  ...
  defmodule ObserverHook do
    def before_lobby_leave(user, lobby) do
      ...
      if pid = Application.get_env(:gamend_core, :before_leave_observer_pid) do
  ...
  test "before_lobby_leave fires while the member's lobby-scoped entries still exist" do
    ...
    Application.put_env(:gamend_core, :before_leave_observer_pid, self())
    previous = Application.get_env(:gamend_core, :hooks_module)
    Application.put_env(:gamend_core, :hooks_module, ObserverHook)

    on_exit(fn ->
      Application.delete_env(:gamend_core, :before_leave_observer_pid)
