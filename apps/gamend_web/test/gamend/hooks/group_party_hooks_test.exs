defmodule Gamend.Hooks.GroupPartyHooksTest do
  @moduledoc """
  ...
  defmodule CaptureHook do
    ...
    defp notify(msg) do
      pid = Process.get(:hooks_test_pid) || Application.get_env(:gamend, :hooks_test_pid)
  ...
  setup do
    ...
    on_exit(fn ->
      ...
      if orig_pid,
        do: Application.put_env(:gamend, :hooks_test_pid, orig_pid),
        else: Application.delete_env(:gamend, :hooks_test_pid)
    end)
  ...
  describe "before_user_update hook" do
    test "fires when update_user is called", %{owner: owner} do
      ...
      {:ok, updated} = Gamend.Accounts.update_user(owner, attrs)
    ...
    test "fires when update_user_display_name is called", %{owner: owner} do
      ...
      {:ok, updated} = Gamend.Accounts.update_user_display_name(owner, attrs)
