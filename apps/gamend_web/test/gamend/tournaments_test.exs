defmodule Gamend.TournamentsTest do
  use Gamend.DataCase
  ...
  # Not @behaviour Gamend.Hooks: implements only the tournament callbacks
  # (they are optional callbacks; hooks dispatch by function export).
  defmodule CaptureHook do
    defp notify(msg) do
      case Application.get_env(:gamend, :hooks_test_pid) do
        nil -> :ok
        pid -> send(pid, msg)
      end
    ...
    def before_tournament_register(user, tournament) do
      ...
      if Application.get_env(:gamend, :tournament_reject_register) do
    ...
    def before_tournament_leave(user, tournament) do
      ...
      if Application.get_env(:gamend, :tournament_reject_leave) do
    ...
    def before_tournament_result(match, winner) do
      ...
      if Application.get_env(:gamend, :tournament_reject_result) do
  ...
  setup do
    ...
    on_exit(fn ->
      if orig_mod,
        do: Application.put_env(:gamend_core, :hooks_module, orig_mod),
        else: Application.delete_env(:gamend_core, :hooks_module)

      Application.delete_env(:gamend, :hooks_test_pid)
      Application.delete_env(:gamend, :tournament_reject_register)
      Application.delete_env(:gamend, :tournament_reject_leave)
      Application.delete_env(:gamend, :tournament_reject_result)
    end)
  ...
  describe "registration" do
    ...
    test "before_tournament_register veto blocks joining" do
      tournament = create_tournament()
      user = AccountsFixtures.user_fixture()
      Application.put_env(:gamend, :tournament_reject_register, true)
    ...
    test "leave withdraws before the draw; hook can veto" do
      ...
      Application.put_env(:gamend, :tournament_reject_leave, true)
      assert {:error, :entry_is_final} = Tournaments.leave_tournament(user, tournament)

      Application.put_env(:gamend, :tournament_reject_leave, false)
  ...
  describe "resolve_match/2" do
    ...
    test "before_tournament_result veto leaves the match open", %{round1: [m1, _]} do
      Application.put_env(:gamend, :tournament_reject_result, true)
