defmodule Gamend.LobbySnapshotsConfigTest do
  @moduledoc """
  ...
  The fix is no longer a per-module env fallback but `Gamend.Settings`: the
  ...
  setup do
    previous_app_env = Application.get_env(:gamend_core, LobbySnapshots)
    ...
    Application.delete_env(:gamend_core, LobbySnapshots)
  ...
  defp restore_app_env(nil), do: Application.delete_env(:gamend_core, LobbySnapshots)
  defp restore_app_env(value), do: Application.put_env(:gamend_core, LobbySnapshots, value)
  ...
  describe "enabled?/0 without a host config block" do
    ...
    test "an explicit app-env block wins: from_env/0 never overwrites it" do
      System.put_env("GAMEND_LOBBY_SNAPSHOTS_ENABLED", "true")
      Application.put_env(:gamend_core, LobbySnapshots, enabled: false)
