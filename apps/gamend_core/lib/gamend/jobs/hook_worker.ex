defmodule Gamend.Jobs.HookWorker do
  @moduledoc false
  # Generic worker that invokes a named hook. Both `Gamend.Jobs.enqueue_hook/3`
  # and `Gamend.Schedule` enqueue this with a `{"hook" => name, "args" => map}`
  ...
  def perform(%Oban.Job{args: %{"hook" => hook} = args}) do
    ...
    case Gamend.Hooks.invoke(hook_fn, [hook_args]) do
      :ok -> :ok
      {:ok, _} -> :ok
