defmodule GameServer.AccountsAvatarHealTest do
  @moduledoc """
  ...
  defp set_profile_url(user, url) do
    {:ok, user} =
      user |> Ecto.Changeset.change(profile_url: url) |> GameServer.Repo.update()
  ...
  test "an intact stored avatar still wins over the provider avatar" do
    user = user_fixture()
    user = set_profile_url(user, stored_avatar_url(user))
    {:ok, _key} = GameServer.Storage.put(stored_avatar_key(user), "png-bytes")
