defmodule GameServer.StorageLocalDirTest do
  @moduledoc """
  ...
  alias GameServer.Storage.Local

  setup do
    previous = Application.get_env(:game_server_core, Local)

    on_exit(fn ->
      if previous,
        do: Application.put_env(:game_server_core, Local, previous),
        else: Application.delete_env(:game_server_core, Local)
    end)
  ...
  test "with nothing configured, the directory is the declared default" do
    Application.delete_env(:game_server_core, Local)
  ...
  test "an explicit directory wins" do
    Application.put_env(:game_server_core, Local, dir: "/data/storage")
  ...
  test "objects written anywhere under the root are listed, not just top level" do
    ...
    Application.put_env(:game_server_core, Local, dir: dir)
    on_exit(fn -> File.rm_rf(dir) end)

    {:ok, _} = GameServer.Storage.put("avatars/user-1/deep/avatar.png", "bytes")

    keys = GameServer.Storage.list_objects() |> Enum.map(& &1.key)

    assert "avatars/user-1/deep/avatar.png" in keys,
           "a nested object must appear in the admin listing"

    assert GameServer.Storage.usage().count == 1
  end
end
