defmodule GameServer.SignalingTest do
  @moduledoc """
  Signaling with no room process.

  The property that matters is that nothing is stored: config comes off the
  lobby and membership comes off `GameServer.Presence`, so any node can serve
  ...
  describe "relay/5" do
    ...
    test "delivers to the target's inbox", %{lobby: lobby, host: host, peer: peer} do
      Phoenix.PubSub.subscribe(GameServer.PubSub, Signaling.inbox(lobby.id, peer.id))
    ...
    test "two non-hosts can talk in a mesh room" do
      ...
      Phoenix.PubSub.subscribe(GameServer.PubSub, Signaling.inbox(mesh.id, b.id))
