defmodule GameServer.Captcha do
  @moduledoc """
  ...
  requests from one IP" (the rate limiter in `GameServerWeb.LiveHelpers`
  ...
  def enabled?, do: GameServer.Settings.get(__MODULE__, :enabled) == true

  @doc "The sitekey to render, falling back to the always-passes dummy."
  @spec site_key() :: String.t()
  def site_key, do: presence(GameServer.Settings.get(__MODULE__, :site_key)) || @test_site_key
  ...
  `GameServerWeb.LiveHelpers.client_ip/1` returns without peer data) is
  ...
  defp secret_key,
    do: presence(GameServer.Settings.get(__MODULE__, :secret_key)) || @test_secret_key
