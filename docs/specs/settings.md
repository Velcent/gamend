...
- [ ] `GameServer.Settings` + `Settings.Provider` in `game_server_core`; every
      core section converted; `Limits.get/1` delegates without changing callers.
- [ ] Env names derived; no hand-written name except the 12 inherited.
- [ ] Three levels (`required: :prod` / `:warn` / omitted) with the `when:` and
...
- [ ] No setting is stricter than today: the three current raises still raise,
      the three current warns still warn.
- [ ] Plugin settings registered on `PluginManager` load, host config winning.
- [ ] ~60 business-logic `System.get_env` calls migrated to `Settings.get/2`;
      `GameServer.Config` and `GameServer.Env` folded in.
- [ ] `host_runtime.exs` under ~30 lines; starter and polyglot on the thin
      `runtime.exs`.
- [ ] `.env.example` generated; `mix gamend.settings.migrate_env` works.
- [ ] `admin_live/settings.ex` + `/admin` card + route + nav +
      `admin_pages_render_test`; `runtime_introspection` scraping deleted.
- [ ] Docs: deployment / scaling / cache_setup / email_setup / payments /
